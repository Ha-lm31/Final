...
# right = red(50), up = red(75), left = green(20), down = red(25)
# right = red(25), up = red(50), left = red(75), down = green(20)
...
defaultRed2 = (defaultGreen + defaultYellow) * 2
defaultRed = (defaultGreen + defaultYellow) * 3
defaultMinimum = 10
defaultMaximum = 60
signals = []
noOfSignals = 4
...
def initialize():
    # Quand 'right' est green, c'est le premier signal (index 0)
    # On applique les red time spécifiques aux autres directions
    ts1 = TrafficSignal(0, defaultYellow, defaultGreen, defaultMinimum, defaultMaximum)  # right (currentGreen)
    signals.append(ts1)
    # up: red = 25
    ts2 = TrafficSignal(defaultRed1, defaultYellow, defaultGreen, defaultMinimum, defaultMaximum)  # up
    signals.append(ts2)
    # left: red = 50
    ts3 = TrafficSignal(defaultRed2, defaultYellow, defaultGreen, defaultMinimum, defaultMaximum)  # left
    signals.append(ts3)
    # down: red = 75
    ts4 = TrafficSignal(defaultRed, defaultYellow, defaultGreen, defaultMinimum, defaultMaximum)  # down
    signals.append(ts4)
    repeat()
    ...
    while True:
        ...
        for i in range(0,noOfSignals):
            if(i == currentGreen):
                ...
                # Correction ici : toujours afficher le temps restant au rouge
                if(signals[i].red == 0):
